...
mirror :: ( Ix a , Ix b ) => Relation a b c -> Relation b a c
mirror r =
...
complement :: ( Ix a , Ix b, Boolean c ) => Relation a b c -> Relation a b c
complement r =
...
difference :: ( Ix a , Ix b, Boolean c )
        => Relation a b c -> Relation a b c ->  Relation a b c
difference r s =
    intersection r $ complement s

-- | Constructs the union \( R \cup S \) of the relations \( R, S \subseteq A \times B \).
union :: ( Ix a , Ix b, Boolean c )
        => Relation a b c -> Relation a b c -> Relation a b c
union r s
...
product :: ( Ix a , Ix b, Ix c, Boolean d )
        => Relation a b d -> Relation b c d -> Relation a c d
product a b =
...
power  :: ( Ix a, Boolean b ) => Int -> Relation a a b -> Relation a a b
...
intersection :: ( Ix a , Ix b, Boolean c )
...
-- \( R \subseteq A \times A \).
reflexive_closure :: (Ix a, Boolean b) => Relation a a b -> Relation a a b
reflexive_closure t =
    union t $ identity $ bounds t

-- | Constructs the symmetric closure \( R \cup R^{-1} \) of the relation
-- \( R \subseteq A \times A \).
symmetric_closure :: (Ix a, Boolean b) => Relation a a b -> Relation a a b
symmetric_closure r =
    union r $ mirror r
...
transitive_closure :: (Ix a, Boolean b) => Relation a a b -> Relation a a b
...
--
-- Formula size: linear in \( |A|^3 \)
transitive_reflexive_closure :: (Ix a, Boolean b) => Relation a a b -> Relation a a b
transitive_reflexive_closure r =
    transitive_closure r `union` identity (bounds r)

-- | Constructs the equivalence closure \( (R \cup R^{-1})^* \) of the relation
-- \( R \subseteq A \times A \).
--
-- Formula size: linear in \( |A|^3 \)
equivalence_closure :: (Ix a, Boolean b) => Relation a a b -> Relation a a b
