...
terminating :: (Ix a, Boolean b) => Relation a a b -> b
...
--     _                   -> return False
-- @
assert_terminating :: (Ix a, MonadSAT s m, b ~ Bit) => Relation a a b -> m ()
...
--
-- Formula size: linear in \( |A|^3 \)
peak :: (Ix a, Boolean b) => Relation a a b -> Relation a a b -> Relation a a b
...
--
-- Formula size: linear in \( |A|^3 \)
valley :: (Ix a, Boolean b) => Relation a a b -> Relation a a b -> Relation a a b
...
--
-- Formula size: linear in \( |A|^3 \)
locally_confluent :: (Ix a, Boolean b) => Relation a a b -> b
locally_confluent r =
...
--
-- Formula size: linear in \( |A|^3 \)
confluent :: (Ix a, Boolean b) => Relation a a b -> b
confluent r =
  let r' = transitive_reflexive_closure r
  in implies (peak r' r') (valley r' r')
...
--
-- Formula size: linear in \( |A|^3 \)
semiconfluent :: (Ix a, Boolean b) => Relation a a b -> b
semiconfluent r =
  let r' = transitive_reflexive_closure r
  in implies (peak r r') (valley r' r')
...
--
-- Formula size: linear in \( |A|^3 \)
convergent :: (Ix a, Boolean b) => Relation a a b -> b
...
--     _                   -> return False
-- @
assert_convergent :: (Ix a, MonadSAT s m, b ~ Bit) => Relation a a b -> m ()
...
point_symmetric :: (Ix a, Equatable b b) => Relation a a b -> b
point_symmetric r
...
--
-- Formula size: linear in \( |A|^3 \)
relative_to :: (Ix a, Boolean b) => Relation a a b -> Relation a a b -> Relation a a b
r `relative_to` s =
...
--
-- Formula size: linear in \( |A|^3 \)
connected :: (Ix a, Boolean b) => Relation a a b -> b
...
is_nf :: (Ix a, Boolean b) => a -> Relation a a b -> b
is_nf x r =
...
--
-- Formula size: linear in \( |A|^3 \)
nf_property :: (Ix a, Boolean b) => Relation a a b -> b
...
--
-- Formula size: linear in \( |A|^3 \)
unique_nfs :: (Ix a, Boolean b) => Relation a a b -> b
unique_nfs r = and $ do
...
--
-- Formula size: linear in \( |A|^3 \)
unique_nfs_reduction :: (Ix a, Boolean b) => Relation a a b -> b
