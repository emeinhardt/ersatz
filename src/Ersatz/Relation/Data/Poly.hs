...
import Ersatz.Bit
import Ersatz.Bits.Poly ( Bits, sumBit, HasBits )
import Ersatz.Codec
import Ersatz.Variable (exists, Variable)
...
newtype Relation a b c = Relation (Array (a, b) c)

instance (Ix a, Ix b, Codec c) => Codec (Relation a b c) where
  type Decoded (Relation a b c) = Array (a, b) (Decoded c)
...
relation :: ( Ix a, Ix b, MonadSAT s m, Variable c ) =>
  ((a,b),(a,b))
  -> m ( Relation a b c )
relation bnd = do
    pairs <- sequence $ do
...
build :: ( Ix a, Ix b )
      => ((a,b),(a,b))
      -> [ ((a,b), c ) ] -- ^ A list of tuples, where the first element represents an element
...
buildFrom :: (Ix a, Ix b)
          => ((a,b),(a,b))
          -> ((a,b) -> c) -- ^ A function that assigns a 'Bit'-value
...
-- >>> bounds r
-- ((0,0),(1,1))
bounds :: (Ix a, Ix b) => Relation a b c -> ((a,b),(a,b))
...
-- >>> indices r
-- [(0,0),(0,1),(1,0),(1,1)]
indices :: (Ix a, Ix b) => Relation a b c -> [(a, b)]
...
assocs :: (Ix a, Ix b) => Relation a b c -> [((a, b), c)]
...
elems :: (Ix a, Ix b) => Relation a b c -> [c]
...
universeSize :: Ix a => Relation a a b -> Int
universeSize r
  | is_homogeneous r =
...
is_homogeneous :: Ix a => Relation a a b -> Bool
is_homogeneous r =
...
-- \( R \subseteq A \times B \).
card :: (Ix a, Ix b, Boolean c, HasBits c c) => Relation a b c -> Bits c
card = sumBit . elems
